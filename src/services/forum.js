...
var auth = {
	Authorization: "Token jYtEGxXc66LzfnCHflISprQQRzZAU5ZODT63PAUx; userId=1"
};
...
export default {
	...
	getForumNews() {
		return axios.get(
			baseUri +
				"discussions?filter[q]=tag:novidades,patchnotes,devlog&sort=-startTime"
		);
	},
	getPatchnotes() {
		return axios.get(
			baseUri + "discussions?filter[q]=tag:patchnotes&sort=-startTime"
		);
	},
	...
	updateUserAvatar(userId, avatarData) {
		return axios.patch(baseUri + "users/" + userId + "/avatar", avatarData, {
			headers: auth
		});
	},
	...
	getUserToken(username, password) {
		return axios.post(baseUri + "token", {
