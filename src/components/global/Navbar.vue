  ...
  export default {
    ...
    methods: {
      ...
      getUserUnreadedMessages: function() {
        ...
        MessagingService.getMessagesTo(this.user.username).then(res => {
          if (oldLeng != res.data.length) {
            ...
            if (this.messagesNotReaded.length > 0) {
              if (oldLeng < res.data.length) {
                ...
                let lastestNotification = localStorage.getItem(
                  "lastestNotification"
                );

                if (
                  lastestNotification != lastestMessage._id &&
                  lastestNotification != null
                ) {
                  this.$notify({
                    group: "main",
                    ...
                    text:
                      "<b>" +
                      lastestMessage.sender.username +
                      "</b> te enviou uma mensagem.",
                    type: "info",
                    duration: 8000
                  });
